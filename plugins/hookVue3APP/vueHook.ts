import type { RendererNode, ComponentInternalInstance } from "vue";
import { logTrace, log, uniqueColor } from "../common-renderer";
declare global {
    interface Window {
        _vueHooked: WeakMap<Element, ComponentInternalInstance[]>;
    }
}
declare global {
    interface Element {
...
export function hookVue3App(__DEV__ = false) {
    ...
    function recordVue(el: Element, app: ComponentInternalInstance) {
        if (el instanceof Text) {
            ...
            el = el.parentElement! as any as Element;
        }
        //将app记录到WeakMap中
...
//     "recent-contact-item",
// ]
// function proxyContext(app: ComponentInternalInstance){
